...
import { getNotesFromLocalStorage, updateNotesInLocalStorage } from "services";
...
function App() {
  ...
  const updateCurrentPageOnNotesUpdate = (type: "save" | "delete") => {
    ...
    // if note is deleted go to previous page if only one note is left on the page and user is on last page
    else if (type === "delete" && notes.length % PER_PAGE === 1 && currentPage === totalPages) {
      setCurrentPage(totalPages - 1);
    }
  };
      ...
      <div className="tile is-flex-wrap-wrap">
        ...
        <div className="tile is-3 p-2">
          <div className="box is-fullwidth">
            <SaveNote setNotes={setNotes} saveCallback={() => updateCurrentPageOnNotesUpdate("save")} />
          </div>
        </div>
      </div>
      {totalPages > 1 && (
